// src/search-service.ts - Sistema de search
...
export class SearchService {
  ...
  // Brave Search (requer API key grátis)
  ...
  async search(query: string, providers: string[] = ['duckduckgo', 'wikipedia']): Promise<any> {
    ...
    for (const provider of providers) {
      try {
        ...
        switch (provider) {
          case 'duckduckgo':
            result = await this.duckDuckGo(query);
            break;
          case 'wikipedia':
            result = await this.wikipedia(query);
            break;
          case 'serpapi':
            result = await this.serpAPI(query);
            break;
          case 'brave':
            result = await this.braveSearch(query);
            break;
        }
